...
crop_factor = 0.5
fps = 4
reduced_img_width = int(crop_factor * (img_width * matrix_width + m_x * (matrix_width - 1)))
reduced_img_height =  int(crop_factor * (img_height * matrix_height + m_y * (matrix_height - 1)))
print('video size after crop: {}x{} (wxh)'.format(reduced_img_width, reduced_img_height))
out = cv2.VideoWriter(topdir + 'timelaps_matrix_{}fps'.format(fps) + ext,
...
for date in [min_date + datetime.timedelta(days=x) for x in range((max_date - min_date).days + 1)]:
    print(date)
    ...
    for hour in range(24):
        ...
        if len(expanded_filename) == 0:
            ...
            # images.append(image)
            x = (hour % matrix_width) * (img_width + m_x)
            y = (hour // matrix_width) * (img_height + m_y)
            imgmatrix[y:y+img_height, x:x+img_width, :] = image
    ...
    imgmatrix_copy = cv2.resize(imgmatrix, (reduced_img_width, reduced_img_height))
    cv2.putText(imgmatrix_copy, date.strftime("%Y-%m-%d"),
    ...
    ret = out.write(np.array(imgmatrix_copy))  #write frame to the output video
